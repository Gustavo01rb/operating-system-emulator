...
void print_line2( int segment){
    std::cout<<"  |\t" << segment <<"\t|\t    None   \t|\t   Disponível   \t|\tNone\t|\t   None   \t|"<<std::endl;
}
void print_line2(Hadware::ContentData data, int segment){
...
void Memory::print_list() const {
    for(int current_segment = 0; current_segment < this->segments; current_segment++ ){
        if(!ram[current_segment].alocated) print_line2(current_segment);
        else print_line2(ram[current_segment], current_segment);
    }
}
